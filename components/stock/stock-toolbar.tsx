...
const VIEW_TABS: {
  ...
  },
  {
    ...
    tone: "emerald",
    hint: "อยู่ในเป้าขายเดือนนี้ของเซลล์คนใดก็ได้ในระบบ VDA แต่ร้านยังไม่เคยสต็อก — สั่งเพิ่มได้เลย",
    hideWhenEmpty: true,
  },
  {
    view: "deadStock",
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between gap-2 border-t border-slate-100 px-4 py-2.5 dark:border-slate-800">
          <Button
            ...
            onClick={onClear}
            // ปุ่มนี้ล้างเฉพาะสิ่งที่อยู่ในเมนูนี้ — แท็บกับปุ่มซ่อนมีปุ่มของตัวเองบนทูลบาร์
            title={
              activeCount === 0
                ? "ยังไม่ได้เลือกแบรนด์หรือกลุ่มสินค้า — ไม่มีอะไรให้ล้าง"
                : "ล้างแบรนด์และกลุ่มสินค้า (ไม่แตะแท็บและปุ่มซ่อนของไม่ขาย)"
            }
          >
            ล้างตัวกรอง
          </Button>
...
export function StockToolbar({
  ...
  /** กำลังค้นหาทั้งที่มีตัวกรองที่**ถูกข้าม**เปิดอยู่ — ต้องบอกให้รู้
   *  ไม่งั้นผู้ใช้เห็นของนอกแท็บโผล่มาแล้วงงว่าแท็บเสียหรือเปล่า
   *  (ปุ่มซ่อนไม่นับ เพราะมันยังทำงานกับผลค้นหา ไม่ได้ถูกข้าม) */
  const searchIgnoresFilters =
    Boolean(search.trim()) &&
    (countActiveFilters(filters) > 0 || filters.view !== "all");
   ...
   *
   *  ตอนค้นหา**ไม่**อยู่ในรายการนี้ — ปุ่มยังทำงานกับผลค้นหา (ดู selectStockRows) */
  const hideNoSalesDisabledReason = !hideNoSalesApplies(filters.view)
    ? filters.view === "target"
    ...
    : noSalesInView === 0
      ? search.trim()
        ? "ผลการค้นหานี้ไม่มีสินค้าที่ไม่ขาย 1 เดือน — ไม่มีอะไรให้ซ่อน"
