...
### 1. คลัง VDA / ร้านค้า
เข้าระบบด้วย **อีเมล + รหัสผ่าน** (บัญชีต้องให้แอดมินเปิดให้ก่อน) แล้วเข้าที่ `/stock`
เห็นสินค้าทุกตัวพร้อม

- **สต็อกคงเหลือ** (หน่วยหีบ + เศษชิ้น)
- **ขายเฉลี่ยต่อวัน** จากยอดขายจริง 7 หรือ 30 วัน (ดูกราฟย้อนหลังได้ 7 / 30 / 90 วัน)
...
### 3. Admin
`/admin` แบ่ง 5 หมวด — **ข้อมูล** (sync, ดูข้อมูลดิบ, ทะเบียนคลัง VDA) · **ร้านค้า**
(อนุมัติบัญชี, MIN/MAX) · **โปรโมชั่น** · **มุมมองทดสอบ** (ดูเป็น VDA/เซลล์คนอื่น) · **ระบบ**
...
| Auth | Microsoft Entra ID (PKCE ฝั่งเบราว์เซอร์) + session cookie ที่เซ็น HMAC สำหรับร้าน |
| Export | ExcelJS |
