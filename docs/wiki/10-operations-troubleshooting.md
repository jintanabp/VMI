...
### เซลล์เข้าสู่ระบบไม่ได้ ("ไม่พบข้อมูลพนักงานใน master ...")

อีเมลนั้นไม่อยู่ใน `cross_salesman_reference_email` และแอดมินยังไม่ได้กำหนดรหัสให้ —
เพิ่มอีเมลอ้างอิงให้รหัสเซลล์ที่ `/admin/system/vda-sales` (มีผลทันที ไม่ต้องรอ sync)
...
ถ้ายังเห็น ให้ตรวจว่าแถวใน `SalesmanEmailAssignment` ถูกลบ/`active=false` จริง และอีเมลนั้นไม่ได้เป็นเจ้าของรหัส
ใน cross_salesman อยู่แล้ว (อีเมลสีเทา "อัตโนมัติ" ในตาราง แก้ที่ไฟล์ต้นทาง)
