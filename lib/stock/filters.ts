 ...
 *
 * **กติกา: กำลังค้นหา = ข้ามแท็บ/แบรนด์/กลุ่มสินค้า** คนที่พิมพ์ชื่อ/รหัสสินค้าต้องการหา
 ...
 *
 * **ข้อยกเว้นเดียว: ปุ่ม "ซ่อนของไม่ขาย 1 เดือน" ยังทำงานตอนค้นหา** — ผู้ใช้ค้นคำกว้าง ๆ
 * เช่นชื่อแบรนด์แล้วได้ของตายปนมาเต็ม ต้องกดซ่อนได้เหมือนตอนไม่ค้นหา เหตุผลของกติกา
 * ข้างบนคือ "อย่าให้ของหายเพราะสิ่งที่ผู้ใช้ลืมว่าเปิดค้างไว้" ซึ่งไม่ตรงกับปุ่มนี้:
 * มันอยู่บนทูลบาร์ ไฟติดเห็นชัด และบอกจำนวนที่ซ่อนอยู่บนตัวปุ่มเอง
 *
 * ยกเว้นสินค้าเป้าขาย (fromTarget) ที่ไม่ถูกซ่อน — ทุกแถวไม่มียอดขายโดยธรรมชาติเพราะ
 * ร้านยังไม่เคยสต็อก ถ้าซ่อนด้วยจะกลายเป็น "ค้นชื่อสินค้าที่ควรมีขายแล้วไม่เจอ"
 * ด้วยเหตุผลเดียวกับที่แท็บ "ควรมีขาย" ไม่ให้ปุ่มนี้ทำงาน (ดู hideNoSalesApplies)
 */
...
>(
  ...
  if (!q) return filterStockRows(rows, opts.filters);

  const hide = opts.filters.hideNoSales;
  return rows.filter((r) => {
    if (!matchesProductSearch(q, r)) return false;
    if (hide && r.noSales30 && !r.fromTarget) return false;
    return true;
  });
}
